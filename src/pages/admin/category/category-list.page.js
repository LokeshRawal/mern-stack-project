...
const AdminCategoryList = () => {
    ...
    const deleteAction = async(id) => {
        try{
            ...
            if(response.status){
                ...
                loadAllCategories({perPage: 10, page: 0});
            }else{
    ...
    const columns = [
        ...
        {
            name: 'Image',
            selector: row => <ImageFormatter url={"/images/category/"+row.image}/>,
            sortable: true,     
        },
        {
            name: 'Featured',
    ...
    const loadAllCategories = useCallback(async(config)=>{
        try{
            let response = await categorySvc.listCategories(config);
            if(response.status){
    ...
    const handlePerPageChange = (newPerpage, page) => {
        ...
        loadAllCategories({perPage: newPerpage, page: page})
    }
    ...
    const handlePageChange = (page) => {
        ...
        loadAllCategories({perPage: paginate.perPage, page:page})
    }
    ...
    return(<>
        <div className="container-fluid px-4">
            ...
            <AdminBreadCrumb path={[{ title: "Category List", url: null }]} />
